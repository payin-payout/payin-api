# Описание api запросов для работы рекуррентных платежей через систему Payin-payout. Для сервисов с поддержкой 3DS
- [Введение](#Введение)
...
amount |нет| Дробное число,больше нуля, дробная часть отделяется точкой, два знака после точки.| Сумма, которая будет временно списана с клиента для подтверждения токена
email |нет |Строка до 50 символов |Адрес электронной почты покупателя
